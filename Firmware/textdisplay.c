...
/*
	...
	Thanks to erafrmu for updating the td_getRowA(row) function to work with displays with four lines.
...
#define _td_delay() { nop(); nop(); nop(); nop(); nop(); nop(); nop(); nop();}

#define _td_set_rs() {TEXTDISPLAY_RS_PORT|=(1<<TEXTDISPLAY_RS_P); nop(); nop();}
#define _td_clr_rs() {TEXTDISPLAY_RS_PORT&=~(1<<TEXTDISPLAY_RS_P); nop(); nop();}

#define _td_set_rw() {TEXTDISPLAY_RW_PORT|=(1<<TEXTDISPLAY_RW_P); nop(); nop();}
#define _td_clr_rw() {TEXTDISPLAY_RW_PORT&=~(1<<TEXTDISPLAY_RW_P); nop(); nop();}

#define _td_set_e() {TEXTDISPLAY_E_PORT|=(1<<TEXTDISPLAY_E_P); nop(); nop();}
#define _td_clr_e() {TEXTDISPLAY_E_PORT&=~(1<<TEXTDISPLAY_E_P); nop(); nop();}
...
void td_putCG(uint8_t ch, uint8_t address) 
{
	td_setCGaddress(address);
	td_put(ch);
}

void td_putStr(const char *str, uint8_t x, uint8_t y) {
	uint8_t i=0;
	while(str[i]!=0) {
